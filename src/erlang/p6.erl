...
chalk(Test, List, MaxNum) when length(List) < MaxNum ->
    ...
    TestList = lists:dropwhile(fun(X) -> (Sqrt < X) end, List),
    %% start testing with smaller numbers (gives higher hit rate)
    RTestList = lists:reverse(TestList),
    %io:format("chalk ~w ~w ~w ~w ~w~n",[Test,Sqrt,List,TestList,RTestList]),
    case is_prime(Test, Sqrt, RTestList) of
	false ->
